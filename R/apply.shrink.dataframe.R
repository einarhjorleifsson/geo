...
apply.shrink.dataframe <- function(data, name.x, name.ind, FUNS = NULL, 
  NA.rm = FALSE, resp.name = NULL, full.data.frame = FALSE, Set = NA, 
  name.res, ...)
  {
  ...
  indices <- list()
  for (i in 1:length(name.ind)) indices[[i]] <- data[, name.ind[i]]
  if (full.data.frame)
  {
  ...
  } else
  {
    for (i in 1:length(FUNS))
    {
      x <- apply.shrink(data[, name.x[i]], indices, FUNS[i])
      if (i == 1) 
        result <- x else result <- cbind(result, x[, ncol(x)])
    }
  }
