...
#' }
#' @export geoworld
geoworld <- function(regions = ".", exact = FALSE, boundary = TRUE, fill = FALSE, 
  color = 1, lwd = 1, lty = 1, plot = TRUE, type = "l", pch = ".", database = "world", 
  ...
  if (geopar$projection == "Lambert")
  {
    ...
    p1 <- list(x = c(geopar$limx[1], mean(geopar$limx), geopar$limx[1], 
      geopar$limx[2]), y = c(geopar$limy[1], geopar$limy[2], geopar$limy[2], 
      geopar$limy[2]))
    limits <- invProj(p1$x, p1$y, geopar$scale, geopar$b0, geopar$b1, 
      geopar$l1, geopar$projection)
    xlim <- c(limits$lon[3], limits$lon[4])
    ylim <- c(limits$lat[1], limits$lat[2])
  } else
  {
    limits <- invProj(geopar$limx, geopar$limy, geopar$scale, geopar$b0, 
  ...
  # parameter checks turn the region names into a list of polygon numbers
  ...
  line <- maps:::mapgetg(database, gon, fill, c(-1e+06, 1e+06), c(-1e+06, 
    1e+06))
  if (length(line$number) == 0) 
    stop("nothing to draw: all regions out of bounds")
  # turn the polyline numbers into x and y coordinates
  if (fill) 
    coord <- maps:::mapgetl(database, line$number, c(-1e+06, 1e+06), 
      c(-1e+06, 1e+06)) else
      {
    l <- abs(line$number)
    if (boundary && interior) 
      l <- unique(l) else if (boundary) 
      l <- l[!match(l, l[duplicated(l)], FALSE)] else l <- l[duplicated(l)]
  ...
  if (doproj)
  {
    coord <- Proj(coord$y, coord$x, geopar$scale, geopar$b0, geopar$b1, 
      geopar$l1, geopar$projection)
    coord$error <- FALSE
  }
  # for filled regions, turn NA breaks at polylines into NA breaks at
  # polygons, deleting polygons for which there is a corresponding NA
  # color
  if (fill)
  {
    gonsize <- line$size
  ...
  if (plot)
  {
    ...
    if (!fill) 
      geolines(data, col = color, lwd = lwd, lty = lty)
  }
  return(invisible())
}
