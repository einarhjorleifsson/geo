...
giveborder <- function(nholes = 0)
{
  ...
  # new
  xba$y <- c(xb$y, xb$y[1])
  # new Read in the data for holes.  Stored in two vectors (added to the
  # border.  xba has NA's and the first point of each point twice and is
  # used for plotting while xb is used for the program 'margh' and has
  # each point only once and no NA's.
  lxv <- c(1:(nholes + 2))
  lxv[] <- 0
  if (nholes > 0)
  {
  ...
  if (nholes == 0) 
    lx1 <- 1
  # xba$x <-c(xba$x,xb$x[lx1]) xba$y <-c(xba$y,xb$y[lx1])
  reg <- invProj(xba$x, xba$y, geopar$scale, geopar$b0, geopar$b1, geopar$l1, 
    geopar$projection)
