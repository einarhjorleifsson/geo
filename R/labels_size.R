...
#' @keywords aplot
#' @export labels_size
labels_size <- function(cont, digits, sizes, xlim = c(0, 1), ylim = c(0, 
  1), fill = F, n, rat, minsym = "<", label.resolution = 0, open = F, 
  lwd = 1, col = 1)
  {
  xlim <- sort(xlim)
  ylim <- sort(ylim)
  ...
  if (!open) 
    polygon(x, y, col = col, border = T) else lines(x, y, col = col, lwd = lwd)
}
