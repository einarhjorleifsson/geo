...
orthproj <- function(pts, curve)
{
  ...
  x <- .C("Curvedist", PACKAGE = "geo", as.double(curve$x), as.double(curve$y), 
    as.double(curve$dist), as.integer(length(curve$x)), as.double(pts$x), 
    as.double(pts$y), as.double(perdist), as.double(pardist), as.integer(length(pts$y)))
  pardist <- x[[7]]
  perdist <- x[[8]]
