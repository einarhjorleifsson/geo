...
#' }
#' @export geoinside
geoinside <- function(data, reg, option = 1, col.names = c("lat", "lon"), 
  na.rm = T, robust = F)
  {
  ...
  regy <- reg[[i1[2]]]
  n <- length(regx)
  k <- (regx[1] != regx[n] || regy[1] != regy[n]) && length(regx) != 
    2
  if (k && !is.na(k))
  {
    regx <- c(regx, regx[1])
  ...
  if (length(reg$x) == 2) 
    reg <- list(x = c(reg$x[1], reg$x[2], reg$x[2], reg$x[1], reg$x[1]), 
  ...
  if (robust)
  {
    ...
    inside <- .C("marghc", PACKAGE = "geo", as.double(data$x), as.double(data$y), 
      as.integer(length(data$y)), as.double(border$x), as.double(border$y), 
      as.integer(length(border$y)), as.integer(border$lxv), as.integer(length(border$lxv)), 
      as.integer(inside), as.double(a), as.double(a1))
    inside <- inside[[9]]
  } else
  {
    # Faster method.
    tmpinside <- rep(0, length(border$lxv))
    inside <- .C("geomarghc", PACKAGE = "geo", as.double(data$x), as.double(data$y), 
      as.integer(length(data$y)), as.double(border$x), as.double(border$y), 
  ...
  } else if (option == 2)
  {
    ...
    return(inside) else if (option == 4) 
    return(1 - inside) else if (option == 5)
    {
    ind <- c(1:length(inside))
    ind <- ind[inside == 0]
    return(ind)
