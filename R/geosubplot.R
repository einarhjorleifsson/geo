...
#' }
#' @export geosubplot
geosubplot <- function(fun, pos, size = c(2, 2), fill, fillcol, ...)
{
  ...
  if (length(pos$lat) == 1)
  {
    ...
    pos <- data.frame(lat = pos$lat + c(-0.5, 0.5) * rlat, lon = pos$lon + 
      c(-0.5, 0.5) * rlon)
  }
  if (!missing(fill))
  {
    if (!missing(fillcol)) 
      geopolygon(pos, col = fillcol) else geopolygon(pos, col = 0)
  }
  pos <- Proj(pos)
  oldpar <- selectedpar()
  par(geopar$gpar)
