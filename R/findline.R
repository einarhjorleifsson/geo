...
findline <- function(x, xb, plot = T)
{
  ...
  xr <- .C("define_multiline", PACKAGE = "geo", as.double(x$x), as.double(x$y), 
    as.double(xb$x), as.double(xb$y), as.double(xr), as.double(yr), 
    as.integer(length(x$y)), as.integer(length(xb$y)), as.integer(nxr), 
    as.integer(li$lx1), as.integer(li$lx2), as.integer(li$nlx), as.integer(plot), 
    as.double(ab), as.double(ab1))
  nxr <- xr[[9]]
  yr <- xr[[6]]
  yr <- yr[1:nxr]
  xr <- xr[[5]]
