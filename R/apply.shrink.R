...
{
  # GJ 9/94.  'apply.shrink' is identical to 'tapply' (see tapply).  But
  # it returns a data.frame were each 'index' represent a column and an
  # extra column for the result of evaluating FUN for the partation on X
  ...
  INDICES <- INDICES[match(sort(unique(all.indices)), all.indices, nomatch = 0), 
    ]
  if (is.character(FUN)) 
    FUN <- getFunction(FUN) else if (mode(FUN) != "function")
    {
    farg <- substitute(FUN)
    if (mode(farg) == "name") 
      FUN <- getFunction(farg) else stop(paste("\"", farg, "\" is not a function", sep = ""))
  }
  X <- split(X, all.indices)
  X.apply <- lapply(X, FUN, ...)
  numb.FUN.value <- length(X.apply[[1]])
  if (numb.FUN.value == 1) 
    X.apply <- data.frame(X = unlist(X.apply)) else X.apply <- data.frame(matrix(unlist(X.apply), ncol = numb.FUN.value, 
