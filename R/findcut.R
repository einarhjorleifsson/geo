...
#' @export findcut
findcut <- function(x, xb, in.or.out)
{
  if (!is.data.frame(x)) 
  ...
  xr <- .C("define_poly", PACKAGE = "geo", as.double(x$x), as.double(x$y), 
    as.double(xb$x), as.double(xb$y), as.double(xr), as.double(yr), 
    as.integer(length(x$y)), as.integer(length(xb$y)), as.integer(nxr), 
    as.integer(mark), as.integer(side), as.double(s), as.double(t), 
    as.double(ab), as.double(ab1), as.integer(in.or.out))
  nxr <- xr[[9]]
  yr <- xr[[6]][1:nxr]
  mark <- xr[[10]][1:nxr]
  side <- xr[[11]][1:nxr]
  ...
  if (i == 0 && length(ind2) == 0) 
    return(invisible())
  if (ind2[1] == 1) 
    ind1 <- c(1:nxr) else ind1 <- c(ind2[1]:nxr, 1:(ind2[1] - 1))
  xr <- xr[ind1]
  yr <- yr[ind1]
  mark <- mark[ind1]
  side <- side[ind1]
  ...
  up <- rep(0, nrow(s))
  pt <- h[s[, 2]] - 1e-04
  i <- geoinside(find.hnit(pt, xb), reg = x, option = 0, col.names = c("x", 
    "y"))
  if (in.or.out == 1)
  {
  ...
  x <- .C("post_filter", PACKAGE = "geo", as.integer(s1[, 2]), as.integer(side), 
    as.integer(up1), as.integer(mark), as.double(xr), as.double(yr), 
    as.integer(buid), as.integer(nrow(s1)), as.double(xb$x), as.double(xb$y), 
    as.integer(length(xb$y)), as.double(xr1), as.double(yr1), as.integer(nxr))
  nxr <- x[[14]]
  xr <- x[[12]][1:nxr]
  yr <- x[[13]][1:nxr]
  ind <- c(1:nxr)
