...
#' @export plvar
plvar <- function(vagram, n = 4, fit = T, type = "p")
{
  if (fit)
  {
    ...
    plot(vagr1, zva, xlim = c(0, max(vagr1)), ylim = c(0, max(zva) * 
      ...
      type = type)
    lines(c(0, vagr1[1]/2, vagr1), spherical(vagram$rang1, vagram$sill, 
      vagram$nugget, c(0, vagr1[1]/2, vagr1)))
    ...
    txt <- c(paste("nugget/sill=", tmp), paste("range = ", as.character(round(vagram$rang1, 
      digits = 2))))
    print(txt)
    text(tloc, txt)
  } else
  {
    plot(c(0, vagram$dist), c(0, vagram$vario), ylim = c(0, max(vagram$vario)), 
      xlab = "Distance", ylab = "Variogram", title = " ")
  }
}
