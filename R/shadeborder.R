...
#' @keywords aplot
#' @export shadeborder
shadeborder <- function(reg, lat, lon, col = 0, col.names = c("lon", "lat"))
{
  ...
  if (length(ind1) == 0 || ind1[1] != 1)
  {
    ...
    if (length(ind1) < 1) 
      ind2 <- length(reg[[col.names[2]]]) else ind2 <- ind1[1] - 1
    ...
    rx <- Proj(rlat, rlon, geopar$scale, geopar$b0, geopar$b1, geopar$l1, 
    ...
    if (length(ind1) > 0)
    {
      ...
      {
        if (length(reg[[col.names[2]]]) - ind1[length(ind1)] < 
          3) 
          return(invisible())
        reg[[col.names[2]]] <- reg[[col.names[2]]][(ind1[1] + 1):length(reg[[col.names[2]]])]
        reg[[col.names[1]]] <- reg[[col.names[1]]][(ind1[1] + 1):length(reg[[col.names[1]]])]
      }
      rx <- Proj(reg, scale = geopar$scale, b0 = geopar$b0, b1 = geopar$b1, 
        l1 = geopar$l1, projection = geopar$projection, col.names = col.names)
      lines(rx, lwd = 2, col = 2)
    }
  } else
  {
    rx <- Proj(reg, scale = geopar$scale, b0 = geopar$b0, b1 = geopar$b1, 
      l1 = geopar$l1, projection = geopar$projection, col.names)
