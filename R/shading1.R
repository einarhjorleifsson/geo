...
#' @keywords aplot
#' @export shading1
shading1 <- function(cont, digits, colors, xlim = c(0, 1), ylim = c(0, 
  1), fill = F, angle, rotate, cex, rat, minsym = "<")
  {
  xlim <- sort(xlim)
  ylim <- sort(ylim)
  if (cex != 0) 
  ...
  ncont <- length(cont)
  if (fill) 
    lbox <- max(ncont + 1, 20) else lbox <- ncont + 1
  boxy <- c(1:lbox)
  boxy <- -boxy/lbox + 1
