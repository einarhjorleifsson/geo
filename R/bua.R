...
bua <- function(nm = 10)
{
  ...
  i1 <- c(0, i1)
  # Part that comes instead of the loop that is too slow.
  ind <- c(3, 0, 2, 0, 4, 0, 1)
  r1 <- rrt - 0.1
  c1 <- crt - 0.1
  rr <- sign(r1) + sign(c1) * 2 + 4
  dir <- ind[rr]
  # dir<- c(1:length(rrt)) for(i in 1:length(rrt)){ if(rrt[i]>0 &&
  # crt[i]>0)dir[i]<-1 if(rrt[i]<= 0 && crt[i]>0)dir[i]<-4 if(rrt[i]<= 0
  # && crt[i]<=0 )dir[i]<-3 if(rrt[i]>0 && crt[i]<=0)dir[i]<-2 }
