...
inside.reg.bc <- function(data, only.sv.1to10 = F, ignore.latlon = F, ignore.area0 = T)
{
  ...
  if (length(index1) > 0)
  {
    ...
    } else
    {
      if (is.na(match("smareitur", names(reitdata)))) 
        reitdata$smareitur <- rep(0, nrow(reitdata)) else reitdata$smareitur[is.na(reitdata$smareitur)] <- 0
