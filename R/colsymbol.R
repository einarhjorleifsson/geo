...
#' @keywords aplot
#' @export colsymbol
colsymbol <- function(lat, lon, z, circles, squares, rectangles, hbars, 
  vbars, perbars, parbars, levels, nlevels, colors, white, n, maxcol, 
  digits, label.location, labels, fill.circles, density, angle, rotate, 
  minsym = "<", label.resolution = 0, col = 1, labels.only = F, open.circles, 
  ...
  if (circles != 0 && !fill.circles)
  {
    ...
    if (density > 0)
    {
      ...
      for (i in 1:length(ind))
      {
        ...
        if (!labels.only)
        {
          polygon(x1, y1, density = ind[i], border = F, angle = angle1, 
          col = col)
          if (border && ind[i] == 0) 
          lines(x1, y1, col = 1)
        }
  ...
  if (vbars != 0 && !fill.circles)
  {
    ...
    mx[2, ] <- x
    my[2, ] <- my[1, ] + r
    for (i in 1:ncol(mx)) lines(mx[, i], my[, i], col = ind[i])
  }
  ...
  if (length(label.location) > 1)
  {
    ...
    if (fill.circles || open.circles)
    {
      ...
      if (open.circles) 
        open <- T
      labels_size(cont1, digits, colors, xlim = label.location$x, 
        ylim = label.location$y, n = n, rat = ein.pr.in, minsym = minsym, 
        label.resolution = label.resolution, open = open, lwd = lwd, 
        col = col)
    } else if (density > 0) 
      shading1(cont1, digits, colors, angle = angle, rotate = rotate, 
        cex = par()$cex, xlim = label.location$x, ylim = label.location$y) else
        {
      if (labels == 1)
      {
        # labels for each contour line.
        labels1(cont1, digits, colors, xlim = label.location$x, 
          ylim = label.location$y)
      } else
      {
        # more of a constant label.
        labels2(cont1, digits, colors, xlim = label.location$x, 
          ylim = label.location$y)
      }
    }
  }
  if (geopar$cont && labels != 0)
  {
    ...
    if (density > 0) 
      shading1(cont1, digits, colors, angle = angle, rotate = rotate, 
        cex = par()$cex, fill = geopar$cont) else
        {
      if (labels == 1)
      {
        # labels for each contour line.
