...
#' @keywords aplot
#' @export subplot
subplot <- function(fun, x, y, size = c(1, 1), vadj = 0.5, hadj = 0.5, 
  pars)
  {
  if (missing(fun)) 
  ...
  if (missing(pars))
  {
    ...
    par(err = -1)
    fin <- par()$fin
    # dimensions of figure, inches mai doesn't deal with pty='s', etc mai
    # <- par()$mai # bottom, left, top, right margins, in inches
    pltin <- par("plt") * fin[c(1, 1, 2, 2)]
    mai <- c(pltin[3], pltin[1], fin[2] - pltin[4], fin[1] - pltin[2])
    # 
    usr <- par()$usr
    # limits in user units : xmin,xmax,ymin,ymax uin paramter does not
    # exist in R.
    ...
    {
      # then x,y represent corners of plot reparameterize to lower left
      # corner, size
      x <- sort(x)
      y <- sort(y)
