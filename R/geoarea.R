...
#' 
#' @export geoarea
geoarea <- function(data, Projection = "Lambert", old.method = F, ngrdpts = 2000, 
  robust = T)
  {
  area <- 0
  ...
  } else
  {
    ...
    for (i in 1:length(data))
    {
      if (Projection == "Lambert") 
        data[[i]] <- lambert(data[[i]]$lat, data[[i]]$lon, mean(data[[i]]$lat), 
          mean(data[[i]]$lon), mean(data[[i]]$lat)) else data[[i]] <- mercator(data[[i]]$lat, data[[i]]$lon, b0 = mean(data[[i]]$lat))
