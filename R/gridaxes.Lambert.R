...
gridaxes.Lambert <- function(limx, limy, scale, b0, xyratio, grid, col, 
  reitur, smareitur, axratio, axlabels, b1, l1, projection, dlat, dlon, 
  col1 = 1)
  {
  ...
  if (!grid) 
    plx1 <- adjust.grd(plx) else plx1 <- plx
  par(err = -1)
  if (grid) 
    lines(plx1$x, plx1$y, col = col)
  # plot grid.  Horizontal grid
  ...
  if (!grid) 
    ply1 <- adjust.grd(ply) else ply1 <- ply
  par(err = -1)
  ...
  if (length(ind1) == 0) 
    latchar <- paste(latchar, "\u00b0", latcha, "'", sep = "") else
    {
    if (floor(dlat) == dlat) 
      latchar[ind1] <- paste(latchar[ind1], "\u00b0") else latchar[ind1] <- paste(latchar[ind1], "\u00b0", "00'", sep = "")
    latchar[-ind1] <- paste(latchar[-ind1], "\u00b0", latcha[-ind1], "'", 
      sep = "")
  }
  if (length(ind2) == 0) 
    lonchar <- paste(lonchar, "\u00b0", loncha, "'", sep = "") else
    {
    if (floor(dlon) == dlon) 
      lonchar[ind2] <- paste(lonchar[ind2], "\u00b0") else lonchar[ind2] <- paste(lonchar[ind2], "\u00b0", "00'", sep = "")
  ...
  if (axlabels)
  {
    ...
    # geoaxis(side=1,pos = plx$x[1, plx$ind],inside=F) print(plx$x[1,
    # plx$ind]) print( ply$y1[indx])
