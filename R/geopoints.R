...
#' }
#' @export geopoints
geopoints <- function(lat, lon = 0, pch = "*", cex = 0.7, col = 1, lwd = 0, 
  outside = FALSE, jitter = NULL, mkh = NULL, csi = NULL)
  {
  geopar <- getOption("geopar")
  if (!is.null(csi)) 
    cex <- cex * csi/0.12  # Compatibility with old program
  ...
  if (!outside)
  {
    ...
    ind <- ind[xx$x > geopar$limx[2] | xx$x < geopar$limx[1] | xx$y < 
      geopar$limy[1] | xx$y > geopar$limy[2] | is.na(xx$x) | is.na(xx$y)]
    if (length(ind) > 0)
    {
      xx$x <- xx$x[-ind]
      xx$y <- xx$y[-ind]
  ...
  if (!is.null(mkh)) 
    points(xx$x, xx$y, pch = pch, col = col, mkh = mkh) else points(xx$x, xx$y, pch = pch, col = col)
  return(invisible())
}
