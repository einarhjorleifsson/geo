...
#' @seealso \code{\link{Closed.curve}}, \code{\link[splines]{spline.des}}.
#' @keywords smooth
#' @export ps
ps <- function(x, df = NULL, knots = NULL, period = c(0, 2 * pi))
{
  intercept <- T
  nx <- names(x)
  x <- as.vector(x)
  ...
  if (!missing(df) && missing(knots))
  {
    ...
    {
      nknots <- 0
      warning(paste("df was too small; have used ", sorder - (3 - 
        intercept)))
    }
    if (nknots > 0)
    {
      knots <- seq(from = 0, to = 1, length = nknots + 2)[-c(1, nknots + 
        2)]
