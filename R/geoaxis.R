...
#' @keywords aplot
#' @export geoaxis
geoaxis <- function(side, pos, dist, dlat = 0.5, dlon = 1, csi = 0.12, 
  ...
  if (inside) 
    m <- -m
  if (side == 2 || side == 4) 
    ratio <- diff(geopar$origin$lon)/geopar$gpar$pin[1] else ratio <- diff(geopar$origin$lat)/geopar$gpar$pin[2]
  if (side == 2 || side == 4)
  {
    if (missing(pos))
    {
  ...
  } else
  {
    ...
    geotext(rep(latpos, length(pos)), pos, txt, adj = 0.5, cex = cex, 
      outside = T, ...)
  }
}
