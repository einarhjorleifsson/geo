...
void geomarghc(x,y,n,xp,yp,lx,nlx,inni,tmpinni)
...
/*      define_poly
        Defines a polygon that cuts an edge 
        Do not allow the same line to cut twice 
*/
...
/*      element
	Program that in connection with a Splus program
	makes shaded contour lines.
*/
     ...
     int     npoly[],ncont;
{
  ...
  double xc12[160],yc12[160],xc23[160],yc23[160],xc13[160],yc13[160],
    rat,rat1;
...
/*      elcont 
        Divides each rectangle into 4 triangles and finds the
...
/*      neigbour
        Function that marks probable points in the neighbourhood 
        of a point that is to be estimated.
*/

double distance(),spherical(),distance_x();

void neighbour(nr,reitur,n,m,npts_in_reit,pts_in_reit,indrt,maxnr,stdcrt,
               stdrrt,dir,i1,li1,list,nlist,ldir,rat,isub,isubgr,subareas)
...
void select_pts(latgr,longr,list,ldir,nlist,lat,lon,maxnr,option,finallist,
                nr,treitur,maxdist,order,dist,xnewint,dst)
     
     double     latgr,longr,*lat,*lon,*maxdist,*dist;
     int        *list,*ldir,nlist,*finallist,*maxnr,option,nr,*treitur,
                *order,*xnewint;
     double  (*dst)();
...
void pointkriging(lat,lon,z,nlat,latgr,longr,zgr,nlatgr,reitur,n,m,
                  pts_in_reit,npts_in_reit,maxnumber,vgr,stdcrt,stdrrt,
                  dir,i1,li1,option,inni,cov,rhgtside,x,indrt,jrt,maxrt,
                  treitur,rat,maxdist,mz,isub,isubgr,subareas,variance,
                  varcalc,rhgtsbck,sill,minnumber,suboption,x_y,degree,
                  lagrange,zeroset)
     
     double *lat,*lon,*latgr,*longr,*z,*vgr,*zgr,*cov,*x,*rhgtside,*maxdist,
            *rat,*mz,*rhgtsbck,*sill,*variance,*lagrange;
     
     int *reitur,*n,*m,*nlat,*nlatgr,*stdcrt,*stdrrt,*dir,*pts_in_reit,
         *npts_in_reit,*option,*inni,*maxnumber,*indrt,*jrt,*maxrt,*i1,
         *li1,*treitur,*isub,*isubgr,*subareas,*varcalc,*minnumber,
         *suboption,*x_y,*degree,*zeroset;
...
void combinert(lat,lon,z,nlat,reitur,pts_in_reit,npts_in_reit,indrt,jrt,maxrt,
	       newlat,newlon,newz,newn,nnewlat,option,fill,fylla,grdlat,
               grdlon,n,minnumber,wsp,nr,order,wlat,rat,wz)
     
     double *lat,*lon,*z,*newlat,*newlon,*newz,*grdlat,*grdlon,*wsp,*wz,
            *wlat,*rat;
     
     int *nlat,*reitur,*pts_in_reit,*npts_in_reit,*indrt,*jrt,*maxrt,
         *nnewlat,*option,*newn,*fill,*fylla,*n,*minnumber,*nr,*order;
...
void variogram(lat,lon,z,d_dist,number,dist,vario,n,ndist,option,
               evennumber,varioa,dista,numbera,zzp,xy)
...
/*     orthproj
       Orthogonal projection of a point x,y 
       on a line segment from x1,y1 to x2,y2.  
*/
...
/*     Curvedist
       find the distance along a curve xc,yc from a point 
       marked by xp,yp. dc stores the distance from the beginning 
       of the curve to point i.   
